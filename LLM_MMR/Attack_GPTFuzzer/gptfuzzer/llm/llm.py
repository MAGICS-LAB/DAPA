...
class ClaudeLLM(LLM):
    ...
    def generate(self, prompt, max_tokens=512, max_trials=10, failure_sleep_time=60):
    ...
    def generate_batch(self, prompts, max_tokens=512, max_trials=10, failure_sleep_time=60):
        results = []
        for prompt in prompts:
...
class GeminiLLM(LLM):
    ...
    def generate(self, prompt, max_tokens=512, max_trials=10, failure_sleep_time=60):
        ...
        for _ in range(max_trials):
            try:
                completion = self.gemini.generate_content(f"{HUMAN_PROMPT} {prompt}{AI_PROMPT}")
